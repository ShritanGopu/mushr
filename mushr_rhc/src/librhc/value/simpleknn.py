...
class SimpleKNN:
    def __init__(self, node, dtype, map):
        self.node = node
        ...
        self.perm_region = utils.map.load_permissible_region(self.node, map)
        h, w = self.perm_region.shape

        nhalton = self.node.get_int("value/simpleknn/nhalton", default=3000)
        map_cache = utils.cache.get_cache_map_dir(self.node, self.map)
    ...
    def set_goal(self, goal, n_neighbors=7, k=3):
        ...
        reachable_nodes = np.fromiter(length_to_goal.keys(), dtype=np.intp)
        reachable_dst = np.fromiter(length_to_goal.values(), dtype=float)

        self.reachable_pts = pts_w_goal[reachable_nodes]
        self.reachable_nodes = reachable_nodes
        self.reachable_dst = reachable_dst
    ...
    def _viz_halton(self):
        ...
        for i, pts in enumerate(hp):
            ...
            c = ColorRGBA()
            c.a = 1.0
            c.g = float(255.0 * self.reachable_dst[i] / max_d)
            p.x = float(pts[0])
            p.y = float(pts[1])
            m.points.append(p)
            m.colors.append(c)

        pub = self.node.create_publisher(Marker, "~/markers", 100)
        pub.publish(m)
