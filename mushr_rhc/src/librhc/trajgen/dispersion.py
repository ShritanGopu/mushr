...
class Dispersion:
    ...
    def __init__(self, node, dtype, motion_model):
        self.node = node
    ...
    def reset(self):
        self.K = self.node.get_int("K", default=62)
        self.T = self.node.get_int("T", default=15)

        self.logger.info("K received: " + str(self.K))
        # Number of resamples in control space
        branching_factor = self.node.get_int(
            "trajgen/dispersion/branching_factor", default=3
        )
        samples = self.node.get_int("trajgen/dispersion/samples", default=5)

        self.logger.info(str(samples))

        # Total number of controls in mother set
        N_mother = samples ** branching_factor

        min_delta = self.node.get_float("trajgen/min_delta", default=-0.34)
        max_delta = self.node.get_float("trajgen/max_delta", default=0.34)

        desired_speed = self.node.get_float("trajgen/desired_speed", default=1.0)
        ...
        dispersion_dir = utils.cache.get_cache_dir(
            self.node, os.path.join("trajgen", "dispersion")
        )
    ...
    def _rollout_ms(self, ms_ctrls):
        ...
        self.logger.info("Finished rolling out mother set" + str(k))
        return ms_poses
    ...
    def _prune_mother_set(self, zero_idx, ms_ctrls, ms_poses):
        ...
        self.logger.info("Visited length: " + str(len(visited)))

        assert len(visited) == self.K
        self.ctrls = self.dtype(self.K, self.T, self.NCTRL)
        selected_indices = list(visited.keys())
        self.ctrls.copy_(ms_ctrls[selected_indices])
