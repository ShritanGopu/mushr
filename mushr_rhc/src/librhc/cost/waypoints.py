...
class Waypoints:
    ...
    def __init__(self, node, dtype, map, world_rep, value_fn):
        self.node = node
        ...
        self.viz_rollouts = self.node.get_bool("debug/flag/viz_rollouts", False)
        self.n_viz = self.node.get_int("debug/viz_rollouts/n", -1)
        self.print_stats = self.node.get_bool("debug/viz_rollouts/print_stats", False)
    ...
    def reset(self):
        ...
        self.dist_w = self.node.get_float("cost_fn/dist_w", default=1.0)
        self.obs_dist_w = self.node.get_float("cost_fn/obs_dist_w", default=5.0)
        self.cost2go_w = self.node.get_float("cost_fn/cost2go_w", default=1.0)
        self.smoothing_discount_rate = self.node.get_float(
            "cost_fn/smoothing_discount_rate", default=0.04
        )
        self.bounds_cost = self.node.get_float("cost_fn/bounds_cost", default=100.0)
        ...
        if self.viz_rollouts:
            ...
            if non_colliding.size()[0] > 0:
                ...
                def print_n(c, poses, ns, cmap="coolwarm"):
                    ...
                    rosviz.viz_paths_cmap(self.node, poses[idx], c[idx], ns=ns, cmap=cmap)
