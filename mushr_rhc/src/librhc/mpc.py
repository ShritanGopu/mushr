...
class MPC:
    ...
    def __init__(self, node, dtype, mvmt_model, trajgen, cost):
        self.node = node
        self.dtype = dtype
        self.goal = None
    ...
    def reset(self, init=False):
        ...
        xy_thresh = self.node.get_float("xy_threshold", default=0.5)
        self.goal_threshold = self.dtype([xy_thresh, xy_thresh])
        self.desired_speed = self.node.get_float("trajgen/desired_speed", default=1.0)
        self.dist_horizon = utils.get_distance_horizon(self.node)
