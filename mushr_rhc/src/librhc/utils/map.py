...
from librhc.utils import cache
import rhctensor
...
def load_permissible_region(node, map):
    ...
    return torch.from_numpy(pr.astype(np.uint8)).type(rhctensor.byte_tensor())
